...
public class runTicTacToe {
	...
	public void run()
	{
		...
		while((result = isEnded())==0) //game loop
		{
			if(turn==1)
			{
				positionTicTacToe player1NextMove = ai1.myAlphabetaAlgorithm(board,1); //1 stands for player 1
				if(makeMove(player1NextMove,1,board))
					turn = 2;
			}
			else if(turn==2)
			{
				positionTicTacToe player2NextMove = ai2.randomnessAIAlgorithm(board,2); //2 stands for player 2
		...
		if(result==1)
		{
			...
			System.out.println("Time to win: "+duration+" milliseconds");
		...
		else if(result==2)
		{
			...
			long duration = (endTime - startTime);
			System.out.println("Time to loss: "+duration+" milliseconds");
			printBoardTicTacToe(board);
		}
		else if(result==-1)
		{
			...
			long duration = (endTime - startTime);
			System.out.println("Time to draw: "+duration+" milliseconds");
			printBoardTicTacToe(board);
		}
		else
		{
